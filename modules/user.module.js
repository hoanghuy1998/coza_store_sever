...
const __basedir = path.join(__dirname, "../resoure/avatars/");
        ...
        db.query(
          ...
          (err, updateUser) => {
            if (err) {
              ...
              `;
              mailer.sendMail(user[0].email, "NEW PASSWORD", content);
...
User.changepassword = async (host, id, data, result) => {
  try {
    db.query("SELECT * FROM listuser WHERE id=?", id, (err, user) => {
      if (err) {
        result({
        ...
        db.query(
          ...
          (err, updateUser) => {
            if (err) {
              ...
              const content = `
              Xin Chào Quý Khách
              Tài Khoản Của Quý Khách Hiện Đã Được Đổi Mật Khẩu Thành Công
              Mật Khẩu Hiện Tại Là :${data.password}
              `;
              mailer.sendMail(user[0].email, "NEW PASSWORD", content);
              result("success");
            }
...
User.postUser = (host, data, result) => {
  db.query(
    "select * from listuser where email='" + data.email + "'     ",
    data,
    (err, x) => {
      if (err) {
        result({
        ...
        const getuseridAsnyc = new Promise((resolve, reject) => {
          ...
          db.query("SELECT * FROM listuser", (err, user) => {
            if (err) {
              ...
              let newuserid;
              if (user.length === 0) newuserid = makeid(9);
              else {
                do newuserid = makeid(9);
